  ...
  return (
    <div className="d-flex pt-4 pb-3 my-3 flex-column justify-content-start align-items-start px-4 box">
            ...
            transactions.map((object, index) => {
              return (
                <Accordion.Item className="my-2" eventKey={index}>
                  ...
                  <Accordion.Body>
                    <a href={`${main}${object[0]}`}>{object[0]}</a>
                    <br />
      ...
      <div className="w-100 d-flex justify-content-between">
        <Button onClick={handleClick} variant="danger mx-1">
          Approve
        </Button>
        <Button onClick={handleRemove} variant="success mx-1">
          Close
        </Button>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="w-75 d-flex justify-content-end">
        {batchAddress.length > 0 && (
          <Button
            variant="light"
            onClick={() => batchBlackList(batchAddress, batchUri)}
          >
            Batch BlackList
          </Button>
        )}
        ...
        {requestItems != null &&
          requestItems.map((e) => {
            return (
              e.address && (
                <Card
                  handleChange={() => handleSelect(e.address, e.metadata)}
                  address={e.address}
                  transactions={e.transactions}
                  url={e.metadata}
                  handleClick={() => {
                    blackList(e.address, e.metadata);
                  }}
                  handleRemove={() => {
                    deleteItem(e.address);
                  }}
                />
              )
            );
