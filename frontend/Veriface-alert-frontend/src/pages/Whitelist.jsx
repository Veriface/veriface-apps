...
const Whitelist = () => {
  ...
  const [message, setMessage] = useState(
    `Whitelist already requested for ${account && account.slice(0, 12)} ...`
  );
  ...
  const requestWhiteList = async () => {
    ...
    if (
      ...
      await Moralis.authenticate({
        signingMessage: `request a whitelist for ${account}`,
      })
        .then(async () => {
       ...
       toast.error("Pending whitelist request exists", {
         position: "top-center",
         autoClose: 5000,
          ...
          {isRequested === false &&
            (!loading ? (
              <Button
                ...
                variant="outline-dark text-white w-100"
              >
                Request Address WhiteList
              </Button>
            ) : (
              <Button variant="outline-dark w-100">
                <Spinner className="text-white" animation="border"></Spinner>
              </Button>
            ))}
          {isRequested === true && (
            <Button variant="outline-dark w-100 py-4  text-white">
              {message}
            </Button>
          )}
        </div>
