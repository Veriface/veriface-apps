...
const Blacklist = () => {
  ...
  const requestBlackList = async () => {
    ...
    if ((await checkIfRequest()) == false) {
      toast.error(`blacklist for ${account} requested already'`, {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    if (tdesc1.length < 20 || tdesc2.length < 20) {
      toast.error("Please add required explanations", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <div className="">
      <div
        id="b-section"
        className="my-5 w-100 d-flex flex-column  align-items-center justify-content-center b-section"
      >
        <div className="w-75 align-items-start main p-5 d-flex flex-column">
          <Alert
            variant="light"
            className="d-flex w-100 flex-column align-items-start alert-bg"
          >
          ...
          <div className="d-flex w-100 my-3">
            <input
              value={tIds1}
              onChange={(e) => {
                setTids1((prev) => (prev = e.target.value));
              }}
              className="form-control w-50"
              placeholder="Fradulent transaction Id"
            />
            <input
              value={tdesc1}
              onChange={(e) => {
                setTDesc1((prev) => (prev = e.target.value));
              }}
              className="form-control w-50 mx-1"
              placeholder="Explanation (20 minimum characters)"
            />
          </div>
          <div className="d-flex w-100 my-3">
            <input
              value={tIds2}
              onChange={(e) => {
                setTids2((prev) => (prev = e.target.value));
              }}
              className="form-control w-50"
              placeholder="Fradulent transaction Id"
            />
            <input
              value={tdesc2}
              onChange={(e) => {
                setTDesc2((prev) => (prev = e.target.value));
              }}
              className="form-control w-50 mx-1"
              placeholder="Explanation (20 minimum characters)"
            />
          ...
          {!loading ? (
            <Button onClick={requestBlackList} variant="dark w-100">
      ...
      <div
        id="b-section-sm"
        className="my-5 w-100 px-2 d-flex flex-column  align-items-center justify-content-center b-section-sm"
      >
        <div className="w-100 align-items-start main px-2 py-5 d-flex flex-column">
          ...
          {!loading ? (
            <Button
              className="py-3"
              onClick={requestBlackList}
              variant="dark w-100"
            >
              Request Address Blacklist
            </Button>
          ) : (
            <Button className="py-3" variant="dark w-100">
              <Spinner animation="border"></Spinner>
            </Button>
          )}
        </div>
